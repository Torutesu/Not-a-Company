...
Audience: Operators setting up and running agents in Not a Company

## 1. What this system does

Agents in Not a Company do not run continuously.  
...
For `claude_local` and `codex_local`, Not a Company assumes the CLI is already installed and authenticated on the host machine.
...
Not a Company stores session IDs for resumable adapters.
...
Not a Company pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Not a Company surfaces this as a warning in environment tests, not a hard error.

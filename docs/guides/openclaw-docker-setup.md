...
How to get OpenClaw running in a Docker container for local development and testing the Not a Company OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Not a Company includes an end-to-end join smoke harness:
...
- probes and prints a Not a Company host URL that is reachable from inside OpenClaw Docker
...
If your Not a Company deployment is `authenticated`, provide auth context:
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Not a Company server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Not a Company URL (typically `http://host.docker.internal:3100`).
- If Not a Company rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm paperclipai allowed-hostname host.docker.internal
```

Then restart Not a Company and rerun the smoke script.

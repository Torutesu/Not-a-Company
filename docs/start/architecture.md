...
Not a Company is a monorepo with four main layers.
...
│
├── skills/                      # Agent skills
│   └── paperclip/               # Core Not a Company skill (heartbeat protocol)
│
├── cli/                         # CLI client
...
3. **Agent process** — Adapter spawns the agent (e.g. Claude Code CLI) with Not a Company env vars and a prompt
4. **Agent work** — The agent calls Not a Company's REST API to check assignments, checkout tasks, do work, and update status
...
Adapters are the bridge between Not a Company and agent runtimes. Each adapter is a package with three modules:
...
- **Control plane, not execution plane** — Not a Company orchestrates agents; it doesn't run them

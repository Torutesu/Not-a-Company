# ClipHub: Marketplace for Not a Company Team Configurations

> The "app store" for whole-company AI teams — pre-built Not a Company configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Not a Company-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Not a Company company (agents, projects, governance) |
...
A complete Not a Company company configuration:
...
Single-agent configurations designed to plug into a Not a Company org:
...
Portable skill files that any Not a Company agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Not a Company's skill loading system
...
interface Listing {
  ...
  paperclipVersionMin: string;     // Minimum Not a Company version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Not a Company company
...
| `POST` | `/api/listings/:id/install` | Install to Not a Company company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Not a Company company (or create new)
  → ClipHub API calls Not a Company API to:
  ...
  → Redirect to Not a Company dashboard with new team running
```
...
### 5.3 Creator: Export from Not a Company → Publish

```
Running Not a Company company → "Export as Blueprint" (CLI or UI)
  → Not a Company exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Not a Company brand blue/purple)
...
| Install flow | One-click deploy to Not a Company company |
...
{
  "targetCompanyId": "uuid",         // Existing Not a Company company
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Not a Company
- **Backend**: Node.js API (or extend Not a Company server)
- **Database**: Postgres (can share Not a Company's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Not a Company auth (or OAuth2)

### 9.2 Integration with Not a Company

ClipHub can be:
- **Option A**: A separate app that calls Not a Company's API to install blueprints
- **Option B**: A built-in section of the Not a Company UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Not a Company UI and API.
...
- [ ] Install flow: blueprint → Not a Company company
...
- [ ] "Export from Not a Company" CLI command

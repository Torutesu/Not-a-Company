...
Not a Company supports two runtime modes with different security profiles.
...
When migrating from `local_trusted` to `authenticated`, Not a Company emits a one-time claim URL at startup:

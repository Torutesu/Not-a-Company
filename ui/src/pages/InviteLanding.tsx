...
function dateTime(value: string, locale: "en" | "ja") {
...
export function InviteLandingPage() {
  ...
  const acceptMutation = useMutation({
    mutationFn: async () => {
      if (!invite) throw new Error(t("invite.unavailableTitle"));
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            {t("invite.bootstrapCompleteDetail")}
          </p>
    ...
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">
          {invite.inviteType === "bootstrap_ceo" ? t("invite.bootstrapPageTitle") : t("invite.joinCompanyTitle")}
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">{t("invite.expires")} {dateTime(invite.expiresAt, locale)}.</p>
        ...
        {invite.inviteType !== "bootstrap_ceo" && (
          <div className="mt-5 flex gap-2">
            {availableJoinTypes.map((type) => (
              ...
              >
                {t("invite.joinAs")} {type === "human" ? t("invite.joinAsHuman") : t("invite.joinAsAgent")}
              </button>
            ))}
          </div>
        )}
          ...
          <div className="mt-4 rounded-md border border-border bg-muted/30 p-3 text-sm">
            ...
            <div className="mt-2">
              <Button asChild size="sm" variant="outline">
                <Link to={`/auth?next=${encodeURIComponent(`/invite/${token}`)}`}>{t("invite.signInCta")}</Link>
              </Button>
            </div>
          </div>
        )}

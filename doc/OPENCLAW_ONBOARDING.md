...
1. Start Not a Company in auth mode.
```bash
...
3. In Not a Company UI, go to `http://127.0.0.1:3100/CLA/company/settings`.
...
5. Approve the join request in Not a Company UI, then confirm the OpenClaw agent appears in CLA agents.
...
- This is a separate approval from Not a Company invite approval. You must approve the pending device in OpenClaw itself.
...
- Ask it to create a new CLA issue in Not a Company with unique title `OPENCLAW_CASE_C_CREATED_<timestamp>`.
- Verify in Not a Company UI that new issue exists.

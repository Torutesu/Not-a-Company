...
Not a Company V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Not a Company-shaped design.

Assumption for this document: Not a Company is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Not a Company objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Not a Company.

The main conclusion is:

- Not a Company should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Not a Company should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Not a Company should use multiple extension classes instead of one generic plugin bag:
...
If Not a Company does this well, the examples you listed become straightforward:
...
Relevant Not a Company files reviewed for current extension seams:
...
It is too dangerous for Not a Company core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Not a Company — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Not a Company should absolutely do this.
...
Not a Company should copy this discipline.
...
Not a Company should adopt this style for plugin actions, automations, and UI schemas.
...
Not a Company can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Not a Company too.

## What Not a Company Should Not Copy Directly
...
Not a Company is a control plane for an operator-managed instance with company objects.
...
Not a Company is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Not a Company should not auto-load arbitrary code from a workspace repo like `.paperclip/plugins` or project directories.
...
For Not a Company, equivalent hooks into:
...
Not a Company should not allow plugins to silently replace:
...
For Not a Company it would be risky because it combines:
...
Not a Company should require an explicit operator install step.

## Why Not a Company Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Not a Company |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted Not a Company instance |
...
That means Not a Company should borrow the good ideas from `opencode` but use a stricter architecture.

## Not a Company Already Has Useful Pre-Plugin Seams

Not a Company has several extension-like seams already:
...
This is good news.
Not a Company does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Not a Company Plugin Model
...
Not a Company already has this pattern implicitly:
...
- `plugin` means integration code that talks to Not a Company through a typed plugin protocol and capability model
...
- invoke explicit Not a Company actions through the API
...
Not a Company should treat plugin installation as a global instance-level action.

Examples:

- install `@paperclip/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Not a Company objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Not a Company already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Not a Company too.
...
A `@paperclipai/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Not a Company instance.

A `create-paperclip-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Not a Company achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Not a Company server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Not a Company's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.
...
## A Concrete SDK Shape For Not a Company
...
export default definePlugin({
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Not a Company entities
    });
...
Any Not a Company plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Not a Company objects, those are plugin data, not enable/disable boundaries.
...
- reuse core tables when the data is clearly part of Not a Company itself
...
If a concept is becoming part of Not a Company's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Not a Company itself
...
### 3. Scope plugin data to Not a Company objects before adding custom schemas

A lot of plugin data naturally hangs off existing Not a Company objects:
...
If Not a Company eventually needs extension-owned tables, I would only allow that for:
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Not a Company.
- Board reviews the diff after a run without leaving Not a Company.
...
This plugin syncs Not a Company work with Linear. It is useful for:

- importing backlog from Linear
- linking Not a Company issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Not a Company and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Not a Company.
- Not a Company issue status changes push to Linear and Linear comments arrive back through webhooks.
...
This plugin syncs Not a Company issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Not a Company issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Not a Company task governance
...
- Plugin settings:
  ...
  - whether new Not a Company issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Not a Company issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Not a Company.
- GitHub webhooks update status/comment state in Not a Company.
- A PR is linked back to the Not a Company issue so the board can follow delivery status.
...
This plugin surfaces external metrics and dashboards inside Not a Company. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Not a Company issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Not a Company plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Not a Company dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Not a Company issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Not a Company core; keep it as additive signal and issue creation
...
This plugin pulls Stripe revenue and subscription data into Not a Company. It is useful for:
...
- Failed payment spikes or churn events can generate Not a Company issues for follow-up.
...
- Stripe data should stay additive to Not a Company core
...
Not a Company should not implement "an OpenCode-style generic in-process hook system."
Not a Company should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Not a Company

...
ClipHub is the public registry where people share, discover, and download Not a Company company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Not a Company what a package registry is to a programming language. Not a Company already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Not a Company — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Not a Company instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Not a Company spec. It contains:
...
```
Downloads the template and creates a new company in your local Not a Company instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Not a Company, export your company as a template, then publish:
...
ClipHub is a **separate service** from Not a Company itself. Not a Company is self-hosted; ClipHub is a hosted registry that Not a Company instances talk to.
...
| **Not a Company CLI** | `paperclipai install`, `paperclipai publish`, `paperclipai cliphub sync` — built into Not a Company |
| **Not a Company UI** | "Browse ClipHub" panel in the Not a Company web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Not a Company) |
| Backend | TypeScript + Hono (consistent with Not a Company) |
| Database | PostgreSQL |
...
5. Not a Company creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Not a Company until it works well
...
## Relationship to Not a Company

ClipHub is **not required** to use Not a Company. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Not a Company what a package registry is to a language runtime: optional, but transformative.
...
- [ ] "Browse ClipHub" panel in Not a Company web UI
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Not a Company's own philosophy)

...
This document defines the V1 design for a Not a Company adapter that integrates with
...
4. synthesized stdout events for Not a Company UI/CLI
...
3. **Agent callback auth to Not a Company** uses a bootstrap exchange flow (no long-lived Not a Company key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Not a Company endpoints, not full SKILL.md prompt injection.
...
## Not a Company Callback + Auth Flow (V1)
...
Goal: avoid putting long-lived Not a Company credentials in prompt text.

Flow:

1. Before launch/follow-up, Not a Company mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Not a Company validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Not a Company API calls.
...
1. Prompt includes a compact instruction to fetch skills from Not a Company.
...
4. Route by Cursor `agentId` to active Not a Company run context.
...
| Skill delivery | local skill dir injection | authenticated fetch from Not a Company skill endpoints |
| Not a Company auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Not a Company context.
3. Add richer PR metadata surfacing in Not a Company UI.

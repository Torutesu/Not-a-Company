...
This spec defines how Not a Company actually runs agents while staying runtime-agnostic.
...
1. Not a Company is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Not a Company manages lifecycle, and on exit Not a Company parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `paperclip.skill` (shared Not a Company skill text block)
